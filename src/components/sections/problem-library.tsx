...
import { SectionHeading, PriorityBadge } from "@/components/shared";
import { categories, problems, type ProblemEntry } from "@/lib/business-data";
...
function SolutionExplorer({ problem, open, onOpenChange }: { problem: ProblemEntry; open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-5xl gap-0 overflow-hidden p-0 sm:rounded-2xl">
        <ScrollArea className="max-h-[92vh]">
          <DialogHeader className="border-b border-slate-100 bg-gradient-to-r from-navy-950 to-navy-800 px-6 py-6 text-left sm:px-8">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">
                    Problem pattern · {problem.rootCauses.length} root causes · {problem.solutions.length} solutions
                  </span>
                </div>
          ...
          <div className="space-y-8 px-6 py-7 sm:px-8">
            ...
            <section>
              <div className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.12em] text-navy-950">
                <Target className="h-4 w-4 text-signal-amber-deep" /> Problem — what is happening
              </div>
            ...
            <section>
              <div className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.12em] text-navy-950">
                <Compass className="h-4 w-4 text-electric-600" /> Root causes — why it happens
              </div>
              <div className="mt-3 grid gap-3 lg:grid-cols-3">
                {problem.rootCauses.map((rc, i) => (
                  <div key={i} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <span className={cn("mt-3 inline-block rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide", frequencyTone[rc.frequency])}>
                      {rc.frequency}
                    </span>
                  </div>
                ))}
            ...
            <section>
              <div className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.12em] text-navy-950">
                <Lightbulb className="h-4 w-4 text-signal-green-deep" /> Solution options — compare approaches
              </div>
              <div className="mt-3 grid gap-3 lg:grid-cols-3">
            ...
            <section className="overflow-hidden rounded-xl bg-navy-950 p-5 sm:p-6">
              ...
              <div className="mt-4 flex items-start gap-3 rounded-lg border border-electric-500/30 bg-electric-500/10 p-3.5">
                <ArrowRight className="mt-0.5 h-4 w-4 shrink-0 text-electric-400" />
                <div>
            ...
            <section>
              <div className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.12em] text-navy-950">
                <ClipboardCheck className="h-4 w-4 text-electric-600" /> Implementation plan
              </div>
              <div className="mt-3 grid gap-3 lg:grid-cols-3">
                    ...
                    <div key={phase} className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                      <div className="mb-3 flex items-center justify-between">
                        ...
                        <span className="text-[11px] font-medium text-slate-500">
                          {phase === "NOW" ? "this week" : phase === "NEXT" ? "2-6 weeks" : "this quarter"}
                        </span>
                      </div>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-white p-5">
                <div className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.12em] text-navy-950">
                  <Target className="h-4 w-4 text-electric-600" /> KPIs
                </div>
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-5">
                <div className="flex items-center gap-2 font-display text-sm font-bold uppercase tracking-[0.12em] text-navy-950">
                  <ShieldAlert className="h-4 w-4 text-red-500" /> Risks &amp; mitigations
                </div>
                <div className="mt-3 space-y-2.5">
    ...
    <section id="problems" className="relative border-t border-slate-200 bg-white">
      <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <SectionHeading
          ...
          title="Common problems, fully diagnosed."
          description="Twelve recurring small-business problems, each mapped from symptom to root causes to compared solutions, implementation steps, KPIs and risks. Recognize yours?"
        />
          ...
          {filtered.map((p, i) => {
            ...
            return (
              <motion.button
                ...
                className="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-card-lift transition-all hover:-translate-y-0.5 hover:border-electric-500/50 hover:shadow-card-lift-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-electric-500"
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-medium text-slate-400">
                    {p.rootCauses.length} causes · {p.solutions.length} solutions
                  </span>
                </div>
      ...
      {selected && (
        <SolutionExplorer problem={selected} open={!!selected} onOpenChange={(v) => !v && setSelected(null)} />
      )}
    </section>
  );
}
